...
contract BookLibrary is Ownable{
    ...
    modifier hasCopies(uint _copies) {
        require(_copies > 0, "Current book must have at least 1 copy!");
        _;
    }
    
    modifier alreadyBorrowed(address _user, uint _bookId) {
        bool _isBorrowed;
        ...
        require(!_isBorrowed, "The user has already borrowed one copy of this book!");
        _;
    }
    ...
    modifier libraryContains(uint _bookId, Book[] memory _library) {
        require(_bookId < _library.length && _bookId >= 0, "Library doesn't have book with this ID.");
        _;
    }
    
    Book[] public books;
    
    function addBook(string calldata _title, uint16 _copies) external onlyOwner containsBook(_title, books) hasCopies(_copies) {
        books.push(Book(_title, _copies));
        uint _bookId = books.length - 1;
        bookToBorrowers[_bookId]; 
    } 
    
    function viewAllBooks() external view returns(string memory) {
    ...
    function borrowBookById(uint _bookId) external hasCopies(books[_bookId].copies) alreadyBorrowed(msg.sender, _bookId) libraryContains(_bookId, books) {
